...
class App extends Component {
  ...
  handleIncrement = (event) => {
    ...
    for (let i = 0; i < shuffledData.length; i++) {
      ...
      if (shuffledData[i].id === parseInt(clickedID)) { 
        ...
        //if image has been clicked before reset the game//check to see if current score is higher than top score
        else {
          this.changeTopScore();
          // this.resetGame();
        }; 
  ...
  //method to check and change the top score if current score is higher
  changeTopScore = () => {
    //if current score is greater than top score, make that the new top score, reset current score to zero
    if (this.state.currentScore > this.state.topScore) {
      this.setState({ topScore: this.state.currentScore });
      this.resetGame();
    }
    else {
      //if not keep top score unchanged, call reset method
      this.resetGame();
    }
  };
